...
- 字幕 ASR 误识别的术语还原，记录到主产物根目录同级的 `asr-corrections/` 目录
  （项目另有约定时以项目为准）；**按讲者（作者）组织**：一个作者一个对照表文件，
  存疑项集中到独立的「存疑清单」文件，与对照表分离；
- 主稿里只保留一行指针（指向作者对照表 + 存疑清单），不要把对照表整段放进主稿；
- 同一讲者/系列的误识别会反复出现，动手前先读该作者的对照表，命中就别重复推导，
  并把新出现的变体补回该作者的文件；
- 本 skill 内不保存任何 ASR 词条，词条只存在于 `asr-corrections/`。
